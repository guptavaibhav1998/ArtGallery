...
app.get('/addtocart/:id',(req,res)=>{
    var painting_id = req.params.id;
    console.log("Painting: "+painting_id);
    var num_of_items=0;
    var result={};


connection.query(
    `select no_of_items from cart where user_id='${loginId}' AND arts_id='${painting_id}' `,
    (err,rows,cols)=>{
        if(err || rows.length==0)
            num_of_items=0;
        else
        num_of_items = Number(rows[0].no_of_items);

        console.log("Successfully fetched num of items");
        console.log(num_of_items);
        if(num_of_items==0) {
            num_of_items+=1;
            console.log("items after update"+num_of_items)
            connection.query(
                `insert into cart values('${loginId}','${painting_id}','${num_of_items}')`,
                (err, rows, cols) => {
                if(err)
                throw err;
                console.log("Successfully added to cart from if block");
            res.redirect('/login');
                }
            )
        }
            ...
            console.log("items after update"+num_of_items)
            connection.query(
            `update cart set no_of_items='${num_of_items}' where arts_id='${painting_id}' AND user_id='${loginId}'`,
            (err, rows, cols) => {
            if(err)
            throw err;
...
app.get('/cart',(req,res)=>{
    ...
    connection.query(
        ...
        (err,rows,cols)=>{
            ...
            link = rows[0].link});
    connection.query(
        `Select cart.arts_id,arts.title,arts.author,arts.price,arts.link,cart.no_of_items from cart,arts where user_id = '${loginId}' AND cart.arts_id = arts.arts_id`,
        (err,rows,cols)=>{
            if(err || rows.length==0) {
                console.log("Doesn't exist");
            }
...
app.get('/orders',(req,res)=>{
    ...
    connection.query(`Select arts.title,arts.author,arts.price,arts.link,orders.no_of_paint from orders,arts where order_cus_id = '${loginId}' AND orders.arts_id = arts.arts_id`,(err, row, col)=>{
        if(err)
            throw err;
